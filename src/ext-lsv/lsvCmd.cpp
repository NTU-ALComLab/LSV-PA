...
void init(Abc_Frame_t* pAbc) {
  ...
  Cmd_CommandAdd(pAbc, "LSV", "lsv_sim_aig", Lsv_CommandSimbdd, 0);
}
...
int Lsv_NtkSimBdd(Abc_Ntk_t* pNtk, char* argv) {  
  ...
  Abc_NtkForEachPo(pNtk, pPo, ithPo) {
    ...
    printf("%s: %d\n", Abc_ObjName(pPo), ddnode==DD_ONE(dd));
  }
  return 1;
}

int Lsv_NtkSimAig(Abc_Ntk_t* pNtk, char* argv) {
  FILE* InputData_File;
  int InputData_Num = 0;
  InputData_File = fopen(argv, "r");
  for (char temp = getc(InputData_File); temp != EOF; temp = getc(InputData_File)) if (temp == '\n') InputData_Num ++;
  fclose(fPtr);
  bool OutputData[Abc_NtkPoNum(pNtk)][InputData_Num];
