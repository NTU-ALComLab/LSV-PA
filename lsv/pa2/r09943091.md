...
7. 當為positive unate時，代表F~x -> Fx 所以不能 (F~x and ~(Fx)) ，so 將F~x和Fx接上and gate, 其中Fx部分有complement, 並assume output=1
8. 當為negitive unate時，代表Fx -> F~x 所以不能 (~(F~x) and Fx) ，so 將F~x和Fx接上and gate, 其中F~x部分有complement, 並assume output=1
